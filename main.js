...
(function () {
	class ImageHandler {
				...
				loadCustomLens: (lens) => {
					if (!(lens instanceof LensBase))
						return console.error(
							"Lens must inherit properties from `LensBase`"
						);
		...
		#loadLens(lens) {
			...
			this.#canvas.onmousemove = (e) =>
				this.#mousemove(e, lens, lensCanvas, this.options);
		}
	...
	class EventEmitter {
		 ...
		 */
		emit(event, props) {
			this.#emittable.dispatchEvent(new CustomEvent(event, props));
		}
   ...
   if (typeof module != "undefined") {
      ...
      (!typeof globals ? globals : window)["PictureJS"] = {
         Lens: Lens,
			LensBase: LensBase,
			Image: ImageHandler,
		};
